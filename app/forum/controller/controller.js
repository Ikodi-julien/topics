...
const forumController = {
  ...
  categories: async (request, response) => {
    ...
    const resCategories = await axios(process.env.STRAPI_URL + '/categories', { method: 'GET' });
  ...
  category: async (request, response) => {
    ...
    const currentCategory = resCategories.data.find(cat => cat.name === catName);
  ...
  topic: async (request, response) => {
    ...
    // Récup du topic
    const resTopic = await axios(process.env.STRAPI_URL + '/topics/' + topicId, { method: 'GET' });
